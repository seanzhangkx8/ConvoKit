...
class TextParser2(TextProcessor):
	"""
	...
	def __init__(self, output_field='parsed', input_field=None, mode='parse',
		...
		if mode in ('tag','tokenize'):
            ...
            # Use NLTK only when user hasn't specified spacy model
			if spacy_nlp is None and sent_tokenizer is None:
			...
			# add sentencizer (only for tag and tokenize mode)
			elif spacy_nlp is not None:
				if 'sentencizer' not in aux_input['spacy_nlp'].pipe_names:
