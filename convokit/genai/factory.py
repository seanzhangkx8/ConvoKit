...
def get_llm_client(provider: str, config_manager, **kwargs):
    ...
    if provider.lower() == "gpt":
        ...
        if GeminiClient is None:
            raise ValueError("Gemini client not available. Please install the 'google-genai' package.")
        return GeminiClient(api_key=api_key, **kwargs)
    elif provider.lower() == "local":
        if LocalClient is None:
            raise ValueError("Local client not available. Please install required dependencies for local model support.")
